...
const createLogger = (options) => {
  ...
  const logDir = options.logDirPath ? options.logDirPath : process.env.WEBSITE_ROOT ? `${process.env.WEBSITE_ROOT}/logs` : `${appRoot}/logs`;
  ...
  const commonOptions = {
    console: {
      ...
      format: combine(
        ...
        printf(
          (msg) =>
            `[${new Date(msg.timestamp).toUTCString()}]: ${msg.label} : - ${msg.level
            }: ${msg.message}`
        )
      ),
    },
